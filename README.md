...
```
python -m analyze --tumor-bam {/path/to/tumor.bam} --normal-bam {/path/to/normal.bam} --case-id my-unique-case --microsatellites-list {/path/to/microsatellites_file} --save-location {/path/to/save/vectors} --model ./model/mimsi_mskcc_impact.model > single_case_analysis.out
```
...
```
python -m analyze --case-list {/path/to/case_list.txt} --microsatellites-list {/path/to/microsatellites_file} --save-location {/path/to/save/vectors} --model ./model/mimsi_mskcc_impact.model > multi_case_analysis.out
```
...
python -m create_data --case-list {path/to/case_list.txt}--microsatellites-list {path/to/microsatellites_list.txt} --save-location {/path/to/save/vectors} --is-labeled 1 > vector_creation.out
```
...
```
python -m evaluate_sample --saved-model {deployment_location}/mimsi/model/mimsi_mskcc_impact.model  --vector-location {/path/to/vectors} > eval_results.out
```
...
```
python -m mi_msi_train_test --name 'model_name' --train-location /path/to/train/vectors/ --test-location /path/to/train/vectors/  --save 1 > train_test.log
```
